...
class TestModuliGeneratorArgParser:
    ...
    def test_argparser_default_values(self):
        ...
        assert args.candidates_dir == str(default_config.candidates_dir.relative_to(default_config.moduli_home))
        assert args.delete_records_on_moduli_write is False
        assert args.key_lengths == default_config.key_lengths
        assert args.log_dir == str(default_config.log_dir.relative_to(default_config.moduli_home))
        assert args.mariadb_cnf == str(default_config.mariadb_cnf.relative_to(default_config.moduli_home))
        assert args.moduli_dir == str(default_config.moduli_dir.relative_to(default_config.moduli_home))
    ...
    @patch('sys.argv', [
        'moduli_generator',
        '--key-lengths', '4096', '8192',
        '--nice-value', '10',
        '--moduli-home', '/custom/path',
        '--delete-records-on-moduli-write',
        '--records-per-keylength', '200'
    ])
...
class TestLocalConfigFunction:
    ...
    def test_local_config_with_none_args(self, mock_is_valid, mock_with_base_dir, mock_argparser):
    ...
    def test_local_config_with_provided_args(self, mock_is_valid, mock_with_base_dir):
        ...
        assert mock_config.candidates_dir == mock_config.moduli_home / 'custom_candidates'
    ...
    @patch('config.default_config.with_base_dir')
    @patch('config.arg_parser.is_valid_identifier_sql')
    def test_local_config_invalid_database_name(self, mock_is_valid, mock_with_base_dir):
        ...
        with pytest.raises(RuntimeError, match="Invalid database name: invalid-db-name!"):
    ...
    @patch('config.default_config.with_base_dir')
    @patch('config.arg_parser.is_valid_identifier_sql')
    def test_local_config_database_name_validation_edge_cases(self, mock_is_valid, mock_with_base_dir):
        ...
        for db_name, is_valid in test_cases:
            ...
            if is_valid:
                ...
                with pytest.raises(RuntimeError, match=f"Invalid database name: {db_name}"):
                    local_config(mock_args)
    ...
    @patch('config.default_config.with_base_dir')
    @patch('config.arg_parser.is_valid_identifier_sql')
    def test_local_config_key_lengths_tuple_conversion(self, mock_is_valid, mock_with_base_dir):
    ...
    @patch('config.default_config.with_base_dir')
    @patch('config.arg_parser.is_valid_identifier_sql')
    def test_local_config_ensure_directories_called(self, mock_is_valid, mock_with_base_dir):
...
class TestConfigIntegrationScenarios:
    ...
    @patch('sys.argv', [
        'moduli_generator',
        '--moduli-home', '/production/moduli',
        '--key-lengths', '4096', '8192',
        '--nice-value', '15',
        '--moduli-db', 'production_moduli',
        '--records-per-keylength', '500',
        '--delete-records-on-moduli-write'
    ])
    @patch('config.default_config.with_base_dir')
    @patch('config.arg_parser.is_valid_identifier_sql')
    ...
    def test_error_handling_integration(self, mock_is_valid, mock_with_base_dir):
        ...
        with pytest.raises(RuntimeError, match="Invalid database name: invalid_db_name!"):
            local_config(args)
    ...
    @patch('sys.argv', [
        'moduli_generator',
        '--key-lengths', '3072', '4096', '7168', '8192',
        '--nice-value', '-20'
    ])
    @patch('config.default_config.with_base_dir')
    @patch('config.arg_parser.is_valid_identifier_sql')
    ...
    @patch('config.default_config.with_base_dir')
    @patch('config.arg_parser.is_valid_identifier_sql')
    def test_config_workflow_with_mocked_argparser(self, mock_is_valid, mock_with_base_dir, mock_argparser):
