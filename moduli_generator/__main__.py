...
def main():
    ...
    generator = ModuliGenerator(nice_value=15, key_lengths=(2048, 2048, 3074))
    ...
    # Get config file path - first check the current directory, then home directory
    config_path = "moduli_generator.cnf"
    if not os.path.exists(config_path):
        config_path = os.path.join(Path.home(), "moduli_generator.cnf")

    # Store Screened Moduli in MariaDB
    db = MariaDBConnector(config_path)
